...
export default function TransactionsPage() {
  ...
  const totals = {
    completed: transactions.filter((t) => t.is_paidOrNah === true).reduce((sum, t) => {
      const totals = parseFloat(t.totals.replace('$', '').replace(',', ''))
      return sum + totals
    }, 0),
    pending: transactions.filter((t) => t.is_paidOrNah === false).reduce((sum, t) => {
      const totals = parseFloat(t.totals.replace('$', '').replace(',', ''))
      return sum + totals
    }, 0),
    failed: transactions.filter((t) => t.is_paidOrNah === false).reduce((sum, t) => {
      const totals = parseFloat(t.totals.replace('$', '').replace(',', ''))
      return sum + totals
    }, 0),
  }
            ...
            <tbody>
              {filteredTransactions.map((transaction) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 text-foreground">{transaction.user.name}</td>
                  ...
                  <td className="py-4 px-6">
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${
                        transaction.is_paidOrNah === true
                          ? 'bg-green-100 text-green-700'
                          : 'bg-yellow-100 text-yellow-700'
                      }`}
                    >
                      {transaction.is_paidOrNah === true ? "completed": "pending"}
                    </span>
                  </td>
                </tr>
              ))}
